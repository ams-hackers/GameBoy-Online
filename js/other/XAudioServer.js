...
XAudioServer.prototype.initializeMediaStream = function () {
  ...
  this.audioHandleMediaStreamProcessing = new ProcessedMediaStream(
...
XAudioServer.prototype.initializeFlashAudio = function () {
  ...
  if (existingFlashload == null) {
    ...
    swfobject.embedSWF(
      XAudioJSsourceHandle.substring(0, XAudioJSsourceHandle.length - 9) +
        "JS.swf",
      "XAudioJS",
      "8",
      "8",
...
function XAudioJSGenerateFlashSurroundString() {
  ...
  for (
    ...
    //Sanitize the buffer:
    XAudioJSBinaryString[XAudioJSTotalSamples++] = String.fromCharCode(
      ((Math.min(
        Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0),
        2
      ) *
        0x3fff) |
        0) +
...
function XAudioJSGenerateFlashStereoString() {
  ...
  for (
    var index = 0;
    index < XAudioJSTotalSamples &&
    XAudioJSResampleBufferStart != XAudioJSResampleBufferEnd;

  ) {
    //Sanitize the buffer:
    XAudioJSBinaryString[index++] = String.fromCharCode(
      ((Math.min(
        Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0),
        2
      ) *
