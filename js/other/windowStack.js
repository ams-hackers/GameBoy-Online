...
windowStack.prototype.getStyleFloatOf = function (
  propertyNameOf,
  JSpropertyNameOf
) {
  try {
    var dirtyValue = window
      .getComputedStyle(this.domObject, null)
      .getPropertyValue(propertyNameOf);
  } catch (error) {
    try {
      var dirtyValue = this.domObject.currentStyle.getAttribute(
        JSpropertyNameOf
  ...
  var cleanValue = parseFloat(
    dirtyValue != null && dirtyValue.length ? dirtyValue : "0"
  );
...
windowStack.prototype.dragEnable = function (event) {
  ...
  this.mousedInsideWindow = mousedWindow
    ? false
    : isDescendantOf(this.domObject, elementAt);
...
windowStack.prototype.interceptOpacity = function () {
  ...
  return sampledOpacity > 0
    ? sampledOpacity
    : 100; /*Dirty opacity first-set for IE*/
};
windowStack.prototype.center = function () {
  ...
  this.domObject.style.left =
    Math.round((docWidth - this.domObject.offsetWidth) / 2 + docLeft) + "px";
  this.domObject.style.top =
    Math.round((docHeight - this.domObject.offsetHeight) / 2 + docTop) + "px";
};
